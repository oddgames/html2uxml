...
namespace ODDGames.Html2Uxml.Editor
{
    ...
    public class Html2UxmlImporterWindow : EditorWindow
    {
        ...
        static int BuildFontAssets(string fontsDir)
        {
            ...
            int count = 0;
            string root = Path.GetDirectoryName(Application.dataPath).Replace('\\', '/');
            var entries = EnumerateFontBuildEntries(fontsDir).ToList();
            ...
            int fallbackUpdates = ApplyFontAssetFallbacks(fontsDir, entries, root);
            if (count > 0 || fallbackUpdates > 0) AssetDatabase.SaveAssets();
            return count;
        }

        static int ApplyFontAssetFallbacks(string fontsDir, List<FontBuildEntry> entries, string root)
        {
            int updates = 0;
            var entriesByAsset = entries
                .GroupBy(e => Path.GetFileName(e.assetPath), StringComparer.OrdinalIgnoreCase)
                .ToDictionary(g => g.Key, g => g.First(), StringComparer.OrdinalIgnoreCase);

            foreach (var entry in entries)
            {
                string assetPath = ToProjectAssetPath(entry.assetPath, root);
                if (string.IsNullOrEmpty(assetPath)) continue;
                var asset = AssetDatabase.LoadAssetAtPath<FontAsset>(assetPath);
                if (asset == null) continue;

                var fallbackFiles = entry.fallbackFontAssets;
                if (fallbackFiles == null || fallbackFiles.Length == 0)
                {
                    string self = Path.GetFileName(entry.assetPath);
                    fallbackFiles = entries
                        .Select(e => Path.GetFileName(e.assetPath))
                        .Where(name => !string.Equals(name, self, StringComparison.OrdinalIgnoreCase))
                        .ToArray();
                }

                var fallbacks = new List<FontAsset>();
                var seen = new HashSet<FontAsset>();
                foreach (var fallbackFile in fallbackFiles)
                {
                    if (string.IsNullOrWhiteSpace(fallbackFile)) continue;
                    string fallbackName = Path.GetFileName(fallbackFile);
                    string fallbackAssetPath = null;
                    if (entriesByAsset.TryGetValue(fallbackName, out var fallbackEntry))
                        fallbackAssetPath = ToProjectAssetPath(fallbackEntry.assetPath, root);
                    else
                        fallbackAssetPath = ToProjectAssetPath(Path.Combine(fontsDir, fallbackName), root);
                    if (string.IsNullOrEmpty(fallbackAssetPath)) continue;
                    var fallback = AssetDatabase.LoadAssetAtPath<FontAsset>(fallbackAssetPath);
                    if (fallback == null || fallback == asset || !seen.Add(fallback)) continue;
                    fallbacks.Add(fallback);
                }

                if (SameFallbackTable(asset.fallbackFontAssetTable, fallbacks)) continue;
        ...
        static string ToProjectAssetPath(string path, string root)
        {
            string norm = path.Replace('\\', '/');
            if (!norm.StartsWith(root + "/", StringComparison.Ordinal)) return null;
            return norm.Substring(root.Length + 1);
        }

        static bool SameFallbackTable(List<FontAsset> current, List<FontAsset> expected)
        {
        ...
        struct FontBuildEntry
        {
            ...
            public string[] fallbackFontAssets;
        }

        [Serializable] class FontAssetManifest { public string defaultCharacters; public FontAssetManifestEntry[] fonts; }
        [Serializable] class FontAssetManifestEntry { public string fontFile; public string fontAsset; public string atlasMode; public string characters; public string[] fallbackFontAssets; }
                ...
                if (manifest?.fonts != null)
                {
                    foreach (var entry in manifest.fonts)
                    {
                        ...
                        yield return new FontBuildEntry
                        {
                            ...
                            fallbackFontAssets = entry.fallbackFontAssets,
                        };
                    }
                    yield break;
                }
