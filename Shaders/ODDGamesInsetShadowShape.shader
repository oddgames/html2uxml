Shader "Hidden/ODDGames/html2uxml/InsetShadowShape"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float EdgeFalloff(float distanceInside, float thickness, float blur)
            {
                distanceInside = max(0.0, distanceInside);
                thickness = max(1.0, thickness);
                if (blur <= 0.001)
                    return step(distanceInside, thickness);
                float linearBand = saturate((thickness - distanceInside) / thickness);
                float gaussian = exp(-(distanceInside * distanceInside) / max(0.0001, 2.0 * blur * blur));
                return saturate(max(linearBand * 0.55, gaussian));
            }

            fixed4 Frag(Varyings input) : SV_Target
            {
                float2 p = input.uv * max(_OverlaySize.xy, 0.0001);
                float shapeDist = RoundedRectDistance(p, _SourceRect, _Radius);
                float inside = 1.0 - smoothstep(-0.25, 0.75, shapeDist);
                ...
                for (int i = 0; i < MAX_SHADOWS; i++)
                {
                    ...
                    float blur = max(0.0, s.z);
                    float thickness = abs(s.w) + max(abs(s.x), abs(s.y)) + blur;
                    bool horizontal = abs(s.x) > 0.001;
                    bool vertical = abs(s.y) > 0.001 || !horizontal;
                    float alpha = 0.0;
                    if (vertical)
                    {
                        float d = s.y >= 0.0 ? p.y - _SourceRect.y : (_SourceRect.y + _SourceRect.w) - p.y;
                        alpha = max(alpha, EdgeFalloff(d, thickness, blur));
                    }
                    if (horizontal)
                    {
                        float d = s.x >= 0.0 ? p.x - _SourceRect.x : (_SourceRect.x + _SourceRect.z) - p.x;
                        alpha = max(alpha, EdgeFalloff(d, thickness, blur));
                    }

                    c.a *= saturate(alpha) * inside;
